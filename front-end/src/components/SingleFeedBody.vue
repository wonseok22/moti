<template>
  <div class="single-feed">
    <!-- 피드의 글본문에 해당되는 부분 -->
    <div class="feed-text">
        <p v-if="isThereImg()" v-line-clamp:20="2">{{ text }}</p>
        <p v-if="isThereImg2()">{{ text }}</p>
    ...
    <div class="feed-btns">
        ...
        <span v-on:click="moveToComment" class="material-symbols-outlined"
        style="color:#A3A3A3;">
            mode_comment
        </span>
...
export default {
    ...
    methods: {
        ...
        moveToComment() {
            this.$router.push({name:'comment', params: {feedId:1}})
        }
    },
}
